...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "omnichannel_fulfillment_kiruthikaa.csv"
OUTPUT_DB: Final[Path] = OUTPUT_DIR / "sales.duckdb"
OUTPUT_CHART: Final[Path] = OUTPUT_DIR / "omnichannel_sales_chart_kiruthikaa.png"
...
def classify_omnichannel_order(enriched: dict[str, Any]) -> dict[str, Any]:
    """Add omnichannel retail fulfillment routing fields.

    This simulates retail fulfillment logic:
      - POS and BOPIS orders are fulfilled from stores.
      - Endless Aisle and ship-to-customer orders are fulfilled from warehouses.
      - Warehouse routing is based on customer/region location.
    """
    region_id = str(enriched.get("region_id", ""))
    customer_id = str(enriched.get("customer_id", "UNKNOWN"))
    device_type = str(enriched.get("device_type", "")).lower()
    quantity = int(enriched.get("quantity", 0))
    is_online_raw = str(enriched.get("is_online", "")).lower()
    is_online = is_online_raw == "true"

    store_location_by_region = {
        "US-CA": "SF",
        "US-TX": "LV",
        "US-MO": "LA",
        "CA-ON": "BUR",
    ...
    store_location = store_location_by_region.get(region_id, "BUR")
    warehouse_location = warehouse_by_region.get(region_id, "ONT")

    if not is_online:
        order_type = "POS_STORE_ORDER"
        fulfillment_source_type = "STORE"
        fulfillment_location = store_location
    elif device_type in {"mobile", "tablet"} and quantity == 1:
        order_type = "BOPIS_ORDER"
        fulfillment_source_type = "STORE_PICKUP"
        fulfillment_location = store_location
    elif quantity >= 2:
        order_type = "ENDLESS_AISLE_ORDER"
        fulfillment_source_type = "WAREHOUSE"
        fulfillment_location = warehouse_location
    else:
        order_type = "SHIP_TO_CUSTOMER"
        fulfillment_source_type = "WAREHOUSE"
        fulfillment_location = warehouse_location

    total_sales_amount = float(enriched.get("total", 0.0))
    return_eligible = "YES" if total_sales_amount <= 250 else "REVIEW_REQUIRED"
    return_status = (
        "RETURN_WINDOW_OPEN" if return_eligible == "YES" else "HIGH_VALUE_REVIEW"
    )

    return {
        "customer_name": f"Customer {customer_id}",
        "customer_email": f"{customer_id.lower()}@example-retail.test",
        "order_type": order_type,
        "fulfillment_source_type": fulfillment_source_type,
...
def process_message(
    ...
    *,
    ...
    omnichannel_fields = classify_omnichannel_order(enriched)
    enriched.update(omnichannel_fields)
...
def consume_messages(
    ...
    *,
    ...
    while consumed_count + skipped_count < MAX_MESSAGES:
        ...
        LOG.info(f"order_type={enriched['order_type']}")
        LOG.info(f"fulfillment_source={enriched['fulfillment_source_type']}")
        LOG.info(f"fulfillment_location={enriched['fulfillment_location']}")
        LOG.info(f"return_status={enriched['return_status']}")
